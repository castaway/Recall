...
O_FILES  = 
H_FILES  = 
MAN1PODS = script/recall_cgi.pl \
	script/recall_create.pl \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) \
	  ...
	  script/recall_fastcgi.pl $(INST_MAN1DIR)/recall_fastcgi.pl.$(MAN1EXT) \
...
$(INST_SCRIPT)/recall_server.pl : script/recall_server.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	...
	$(CP) script/recall_server.pl $(INST_SCRIPT)/recall_server.pl
	$(FIXIN) $(INST_SCRIPT)/recall_server.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/recall_server.pl

$(INST_SCRIPT)/recall_create.pl : script/recall_create.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/recall_create.pl
	$(CP) script/recall_create.pl $(INST_SCRIPT)/recall_create.pl
	$(FIXIN) $(INST_SCRIPT)/recall_create.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/recall_create.pl

$(INST_SCRIPT)/recall_fastcgi.pl : script/recall_fastcgi.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/recall_fastcgi.pl
...
$(INST_SCRIPT)/recall_test.pl : script/recall_test.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	...
	$(CP) script/recall_test.pl $(INST_SCRIPT)/recall_test.pl
	$(FIXIN) $(INST_SCRIPT)/recall_test.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/recall_test.pl

$(INST_SCRIPT)/recall_cgi.pl : script/recall_cgi.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/recall_cgi.pl
	$(CP) script/recall_cgi.pl $(INST_SCRIPT)/recall_cgi.pl
	$(FIXIN) $(INST_SCRIPT)/recall_cgi.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/recall_cgi.pl
...
catalyst_par :: all
	$(NOECHO) $(PERL) -Ilib -Minc::Module::Install -MModule::Install::Catalyst -e"Catalyst::Module::Install::_catalyst_par( '', 'Recall', { CLASSES => [], PAROPTS =>  {}, ENGINE => 'CGI', SCRIPT => '', USAGE => q## } )"
